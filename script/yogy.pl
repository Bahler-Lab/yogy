...
%species_hrefs = (Ath => 'http://www.arabidopsis.org/servlets/Search?type=general&search_action=detail&method=1&sub_type=gene&name=',
                  ...
                  Pfa => 'http://old.genedb.org/genedb/Search?submit=Search+for&organism=malaria&name=',
                  Rno => 'http://rgd.mcw.edu/tools/genes/genes_view.cgi?id=',
                  Sce => 'http://db.yeastgenome.org/cgi-bin/locus.pl?locus=',
                  Spo => 'http://old.genedb.org/genedb/Search?submit=Search+for&organism=pombe&name=');
...
sub do_summary {
    ...
    if ($species eq 'S._pombe') {
        ...
        $href  = "http://old.genedb.org/genedb/Search?submit=Search+for&organism=pombe&name=$sys_id";
        $title = 'GeneDB';
        ...
        $href  = "http://old.genedb.org/genedb/Search?submit=Search+for&organism=malaria&name=$sys_id";
        $title = 'GeneDB';
    ...
    if ($sys_id =~ /^ENS/) {
        ...
        $href  = "http://nov2005.archive.ensembl.org/$ensembl_species{$species_id}/protview?peptide=$sys_id";
        $title = 'Ensembl';
    ...
    if ($uni_id) {
        ...
        $href = "http://www.uniprot.org/uniprot/?$gene_info->[5]";
    ...
    if ($ens_query) {
        ...
        if ($title ne 'Ensembl') {
            ...
            $href  = "http://nov2005.archive.ensembl.org/$ensembl_species{$species_id}/protview?peptide=$ens_query";
    ...
    if ($species eq 'S._pombe') {
        ...
        my $href3 = "http://www.bahlerlab.info/cgi-bin/SPGE/geexview?group=3&q=$new_id&scale=log2&scale_min=-2&scale_max=2";
        my $href2 = "http://www.bahlerlab.info/cgi-bin/SPGE/geexview?group=2&q=$new_id&scale=linear&scale_auto=on";
        my $href1 = "http://www.bahlerlab.info/cgi-bin/SPGE/geexview?group=1&q=$new_id&scale=linear&scale_auto=on";
...
sub do_kogs {
    ...
    foreach my $ko (@kog_olc) {
     ...
     #   Removed link to kog classification pages
        $func_details .=
    ...
    foreach my $prot (@protein_id) {
        ...
        if (($go_term eq 'Yes') or ($go_final eq "Yes")) {
        ...
        my $href3 = "http://www.uniprot.org/uniprot/$uni_id";
 ...
 PROT:
    ...
    PID:
        ...
        if ($prot->[2] =~ /^ens/) {
            ...
            $href  = "http://May2010.archive.ensembl.org/$ensembl_species{$species_id}/Gene/Summary?g=$search_id";
            $title = 'Ensembl';
            ...
            $href  = "http://www.uniprot.org/uniprot/$search_id";
            $title = 'UniProt';

        } elsif ($prot->[2] eq 'sanPF') {

            $href  = "http://old.genedb.org/genedb/Search?submit=Search+for&organism=malaria&name=$search_id";
            ...
            $href= "http://www.arabidopsis.org/servlets/Search?type=general&search_action=detail&name=$search_id&sub_type=protein";
            $title = 'NCBI';
            ...
            $href  = "http://old.genedb.org/genedb/Search?submit=Search+for&organism=pombe&name=$search_id";
...
sub do_homologene {
 ...
 PROT:
    foreach my $prot (@homologene_results) {
        ...
        my $href2 = "http://www.uniprot.org/uniprot/$uni_id";
 ...
 PROT:
    foreach my $prot (@orthomcl_results) {
        ...
        if ($prot->[1] eq 'Aga' or
            ...
            $href  = "http://nov2005.archive.ensembl.org/$ensembl_species{$species_id}/protview?peptide=$new_term";
            $title = 'Ensembl';
        ...
        my $href2 = "http://www.uniprot.org/uniprot/$uni_id";
        # my $href3 = "http://orthomcl.cbil.upenn.edu/cgi-bin/OrthoMclWeb.cgi?rm=sequence&accession=$search_id&taxon=$prot->[1]";

         my $href3 = "http://orthomcl.org/cgi-bin/OrthoMclWeb.cgi?rm=sequenceQueryForm&type=keyword";
        my $tax_id = $species_tax_rev{$species_id};
...
sub do_vals {
    ...
    foreach my $prot (@curated_results) {
        ...
        my $href  = "http://old.genedb.org/genedb/Search?submit=Search+for&organism=pombe&name=$sp_id";
        my $title = 'GeneDB';
...
sub print_form {
    ...
    print p(submit('Submit'), defaults('Reset') );
...
# Commented out to include homo sapeins in GO results
#23-09-10 sanjay khadayate
...
sub get_go_string {
    ...
    my $uni_href  = "http://www.uniprot.org/uniprot/";
