...
open(GENESPRING, "< ./Fission_yeas_annotations.txt")
...
while (<GENESPRING>) {
    ...
    if (defined ($temp[7])) {
        ...
        $genespring_hash{lc($temp[7])} =$temp[0].'@'.$temp[2] if $temp[7] ne '';
...
open(SWISS, "< ./gp2swiss.txt")
    or die "couldn't open file => gp2swiss.txt\n";
...
open(GENEDB, "< ./pompep.txt")
    or die "couldn't open file => pompep\n";
...
open(ORTHOLOGS, "< ./yeast_ortho.txt")
...
open(HOMOLOGENE, "./homologene.data")
    or die "Couldn't Open File => homologene.data\n";
